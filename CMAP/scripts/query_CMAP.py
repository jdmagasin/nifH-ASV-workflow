...
# Get the full path to teh currently running script
...
def colocalize_data(input_data, targets) -> pd.DataFrame:
    """
    ...
    colocalized_data: DataFrame = pd.concat(objs=colocalized_dfs, axis=1)
    return colocalized_data


def compress_and_save(df, output_path) -> None:
    """
...
def main() -> None:
    ...
    try:
        ...
        input_data: pd.DataFrame = pd.read_csv(
            filepath_or_buffer=args.input, sep=",", comment="#"
        )
    ...
    """
        ...
        # Colocalize data
        colocalized_data: pd.DataFrame = colocalize_data(
...
was made and '{args.output}' was not written! See logs for details...
"""
            )
